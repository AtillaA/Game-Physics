...
RigidBodySystemSimulator::RigidBodySystemSimulator(): Simulator(), m_Mouse(), m_Track_mouse(), m_Old_track_mouse(),
                                                      m_Test_case(0) {
}
...
void RigidBodySystemSimulator::notifyCaseChanged(const int test_case) {
...
void RigidBodySystemSimulator::externalForcesCalculations(const float time_elapsed) {
	...
	if (mouse_diff.x != 0 || mouse_diff.y != 0) {
		auto tmp = DUC->g_camera.GetWorldMatrix() * DUC->g_camera.GetViewMatrix();
		auto world_view_inv = Mat4(tmp);
...
void RigidBodySystemSimulator::simulateTimestep(float time_step) { m_Rigid_body_system.simulateTimestep(time_step); }

uint32_t RigidBodySystemSimulator::getNumberOfRigidBodies() const { return m_Rigid_body_system.m_Rigid_bodies.size(); }

Vec3 RigidBodySystemSimulator::getPositionOfRigidBody(const uint32_t i) const {
	return m_Rigid_body_system.m_Rigid_bodies[i].m_Position;
}

Vec3 RigidBodySystemSimulator::getLinearVelocityOfRigidBody(const uint32_t i) const {
	return m_Rigid_body_system.m_Rigid_bodies[i].m_Linear_velocity;
}

Vec3 RigidBodySystemSimulator::getAngularVelocityOfRigidBody(const uint32_t i) const {
	return m_Rigid_body_system.m_Rigid_bodies[i].m_Angular_velocity;
}

void RigidBodySystemSimulator::applyForceOnBody(uint32_t i, const Vec3& loc, const Vec3& force) {
...
void RigidBodySystemSimulator::setOrientationOf(const uint32_t i, const Quat& orientation) {
	m_Rigid_body_system.setOrientationOf(i, orientation);
}

void RigidBodySystemSimulator::setVelocityOf(const uint32_t i, const Vec3& velocity) {
