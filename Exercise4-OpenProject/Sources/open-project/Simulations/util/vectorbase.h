...
namespace GamePhysics {
	...
	public:
		...
		inline DirectX::XMVECTOR toDirectXVector() const { return DirectX::XMVectorSet(x, y, z, 1); }
...
#define VECTOR_EPSILON (1e-10)
	...
	/*************************************************************************
	  ...
	  */
	template <class Scalar>
	inline vector3Dim<Scalar>::vector3Dim(void) { value[0] = value[1] = value[2] = 0; }
	...
	template <class Scalar>
	inline vector3Dim<Scalar>
	vector3Dim<Scalar>::operator-() const { return vector3Dim<Scalar>(-value[0], -value[1], -value[2]); }
	...
	template <class Scalar>
	inline vector3Dim<Scalar>
	operator+(float s, vector3Dim<Scalar> v) { return v + s; }

	template <class Scalar>
	inline vector3Dim<Scalar>
	operator+(double s, vector3Dim<Scalar> v) { return v + s; }

	template <class Scalar>
	inline vector3Dim<Scalar>
	operator+(int s, vector3Dim<Scalar> v) { return v + s; }
	...
	template <class Scalar>
	inline Scalar&
	vector3Dim<Scalar>::operator[](unsigned int i) { return value[i]; }
	...
	template <class Scalar>
	inline const Scalar&
	vector3Dim<Scalar>::operator[](unsigned int i) const { return value[i]; }
	...
	inline Real normHelper(const vector3Dim<Real>& v) { return norm(v); }
	inline Real normHelper(const Real& v) { return (0. < v) ? v : -v; }
	inline Real normHelper(const int& v) { return (0 < v) ? (Real)(v) : (Real)(-v); }
	 ...
	 */
	template <class Scalar>
	inline Scalar normNoSqrt(const vector3Dim<Scalar>& v) { return v[0] * v[0] + v[1] * v[1] + v[2] * v[2]; }
	...
	inline Scalar normalize(vector3Dim<Scalar>& v) {
		Scalar norm;
		Scalar l = v[0] * v[0] + v[1] * v[1] + v[2] * v[2];
		if (fabs(l - 1.) < VECTOR_EPSILON * VECTOR_EPSILON) { norm = 1.; }
		else if (l > VECTOR_EPSILON * VECTOR_EPSILON) {
			norm = sqrt(l);
			Scalar fac = 1. / norm;
			v[0] *= fac;
	 ...
	 */
	template <class Scalar>
	inline vector3Dim<Scalar> refractVector(const vector3Dim<Scalar>& t, const vector3Dim<Scalar>& normal, Scalar nt,
	                                        Scalar nair, int& refRefl) {
		Scalar eta = nair / nt;
	...
	inline void rgbToHsv(vector3Dim<Scalar>& V) {
		...
		h = 0;
		if (s > 0) {
			if (maxindex == 1) { h = ((V[1] - V[2]) / delta) + 0.0; }
			if (maxindex == 2) { h = ((V[2] - V[0]) / delta) + 2.0; }
			if (maxindex == 3) { h = ((V[0] - V[1]) / delta) + 4.0; }
			h *= 60.0;
			if (h < 0.0) h += 360.0;
		}
	...
	template <class Scalar>
	std::string vector3Dim<Scalar>::toString() {
		char buf[256];
	...
	// helper function for output
	template <class Vec>
	bool intVecIsEqual(Vec a, Vec b) { return a[0] == b[0] && a[1] == b[1] && a[2] == b[2]; }
	...
	template <class T>
	inline nVec3i vec2I(T v) { return nVec3i((int)v[0], (int)v[1], (int)v[2]); }

	template <class T>
	inline nVec3i vec2I(T v0, T v1, T v2) { return nVec3i((int)v0, (int)v1, (int)v2); }

	template <class T>
	inline nVec3d vec2D(T v) { return nVec3d(v[0], v[1], v[2]); }

	template <class T>
	inline nVec3i vec2D(T v0, T v1, T v2) { return nVec3d((double)v0, (double)v1, (double)v2); }

	template <class T>
	inline nVec3f vec2F(T v) { return nVec3f(v[0], v[1], v[2]); }

	template <class T>
	inline nVec3i vec2F(T v0, T v1, T v2) { return nVec3f((float)v0, (float)v1, (float)v2); }

	template <class T>
	inline nVec3i vecround(T v) { return nVec3i((int)round(v[0]), (int)round(v[1]), (int)round(v[2])); }
	...
	/* convert to Real vec */
	template <class T>
	inline Vec3 vec2R(T v) { return Vec3(v[0], v[1], v[2]); }
	...
	//--------------------------------------------------------------------------------
	template <class Scalar>
	inline Scalar vector3Dim<Scalar>::maxComponent(void) const { return VMAX(*this); }

	//--------------------------------------------------------------------------------
	template <class Scalar>
	inline Scalar vector3Dim<Scalar>::minComponent(void) const { return VMIN(*this); }
